const pokémon = Object.freeze([
    ...
    // check solution for this one. might be missing Moltres
    ...
    // check solution. my answer produces all names but looks a little weird

    const PkmnId99 = pokémon.filter(p => p.id > 99).map(p => p.name);
    console.log(PkmnId99)

    const poisonPkmn = pokémon.filter(p => p.types == "poison").map(p => p.name);
